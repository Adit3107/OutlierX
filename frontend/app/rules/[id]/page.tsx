...
import { RuleDetailPageContent } from '../../../components/rules/rule-console';
      ...
      <section className="w-full max-w-sm rounded-md border border-border bg-surface p-5">
        <div className="flex items-center gap-3 border-b border-border pb-4">
          ...
          <div>
            ...
            <p className="mt-1 font-mono text-xs uppercase text-muted-foreground">Rule detail</p>
          </div>
        </div>
        <p className="mt-4 text-sm text-muted-foreground">Authenticate to inspect this rule.</p>
...
export default function RuleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <SignedIn>
        <RuleDetailPageContent ruleId={resolvedParams.id} />
      </SignedIn>
    </>
  );
}
